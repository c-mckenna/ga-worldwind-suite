...
public class BingLayer extends BasicMercatorTiledImageLayer
{
	...
	public BingLayer(Dataset dataset)
	{
		super(makeLevels(dataset));
		if (dataset == null)
			throw new NullPointerException("Dataset cannot be null");
		this.dataset = dataset;
		this.setValue(AVKey.DISPLAY_NAME, "Microsoft Virtual Earth " + dataset.label);
	...
	protected static LevelSet makeLevels(Dataset dataset)
	{
		...
		params.setValue(AVKey.DATA_CACHE_NAME, "Microsoft Virtual Earth/" + dataset.label);
		params.setValue(AVKey.SERVICE, "http://ecn.t0.tiles.virtualearth.net/tiles/");
		...
		params.setValue(AVKey.DISPLAY_NAME, "Microsoft Virtual Earth " + dataset.label);
	...
	private static class URLBuilder implements TileUrlBuilder
	{
		...
		{
			String quadkey = tileToQuadKey(tile.getColumn(), tile.getRow(), tile.getLevelNumber() + 2);
			URL url =
					new URL(tile.getLevel().getService() + tile.getLevel().getDataset() + quadkey + ".jpeg?g=2852&mkt=");
			return url;
		}
	}
