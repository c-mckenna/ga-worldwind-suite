...
/**
 * A class that holds the render parameters associated with an animation (e.g.
 * frame rate, dimensions etc.)
 * 
...
public class RenderParameters implements AnimationObject, XmlSerializable<RenderParameters>, Cloneable
{
	...
	public static final String DEFAULT_RENDER_EXTENSION = "tga";
	...
	/**
	 * A percentage scaler applied to the base image dimensions to obtain the
	 * render dimensions
	 */
	...
	/**
	 * The extension for rendering the animation.
	 */
	private String renderExtension = DEFAULT_RENDER_EXTENSION;
	...
	/**
	 * @return The dimensions of the output render (after
	 *         {@link #imageScalePercent} is applied to the base
	 *         {@link #imageDimension})
	 */
	...
	public void setRenderExtension(String renderExtension)
	{
		this.renderExtension = renderExtension;
	}

	public String getRenderExtension()
	{
	...
	public Element toXml(Element parent, AnimationFileVersion version)
	{
		...
		if (renderDestination != null)
		{
			XMLUtil.appendText(result, constants.getRenderDestinationElementName(), renderDestination.toURI()
					.toString());
		}
		if (startFrame != null)
		{
	...
	public RenderParameters fromXml(Element element, AnimationFileVersion version, AVList context)
	{
		...
		case VERSION020:
		{
			...
			result.setFrameRate(XMLUtil.getInteger(element, constants.getFrameRateElementName(), DEFAULT_FRAME_RATE,
					null));
			...
			result.setDimensionsLocked(XMLUtil.getBoolean(element, constants.getLockedDimensionsElementName(),
					DEFAULT_LOCKED_DIMENSIONS));
			...
			result.setDetailLevel(XMLUtil.getDouble(element, constants.getDetailLevelElementName(),
					DEFAULT_DETAIL_LEVEL));

			result.setRenderAlpha(XMLUtil.getBoolean(element, constants.getRenderAlphaElementName(),
					DEFAULT_RENDER_ALPHA));

			result.setImageScalePercent(XMLUtil.getInteger(element, constants.getImageScalePercentElementName(),
