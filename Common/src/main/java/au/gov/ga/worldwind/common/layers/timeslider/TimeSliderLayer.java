 ...
 ******************************************************************************/
package au.gov.ga.worldwind.common.layers.timeslider;
...
public class TimeSliderLayer extends AbstractLayer
{
			...
			if (referencePosition != null)
			{
				...
				if (!Double.isNaN(pot))
				{
					...
					if (!dc.isPickingMode())
					{
						...
						// Draw label
						String label = String.format("%.0f ", divSize);
						gl.glLoadIdentity();
						gl.glDisable(GL.GL_CULL_FACE);
						drawLabel(dc, label, locationSW.add3(new Vec4(divWidth * scale / 2 + (width - divWidth) / 2,
								height * scale, 0)));
					}
					else
					{
